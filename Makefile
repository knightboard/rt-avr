...
gdb: $(TARGET).hex
	simulavr -d atmega16 -c 8000000 -g -P simulavr-disp 1>/dev/null 2>&1 &
	sleep 1
	avr-gdb -q -tui $(TARGET).elf -ex 'target remote localhost:1212'
...
$(INC) : $(INC).h
	$(CC) -mmcu=$(MCU) -E -x assembler-with-cpp -o $(INC) $(INC).h
